...
func testMetricToManyVolumes(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, metricDBTypes, true, metricColumnsWithDefault...); err != nil {
	...
	randomize.Struct(seed, &b, volumeDBTypes, false, volumeColumnsWithDefault...)
	randomize.Struct(seed, &c, volumeDBTypes, false, volumeColumnsWithDefault...)
...
func testMetricToManyAddOpVolumes(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, metricDBTypes, false, strmangle.SetComplement(metricPrimaryKeyColumns, metricColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*Volume{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, volumeDBTypes, false, strmangle.SetComplement(volumePrimaryKeyColumns, volumeColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
...
func testMetricToOneCategoryUsingCategory(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, metricDBTypes, true, metricColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Metric struct: %s", err)
	}
