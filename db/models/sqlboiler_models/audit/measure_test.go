...
func testMeasuresLive(t *testing.T) {
	all, err := Measures(dbMain.liveDbConn).All()
	if err != nil {
		t.Fatalf("failed to get all Measures err: ", err)
	}
	...
	bf := &bytes.Buffer{}
	dumpCmd := exec.Command("psql", `-c "COPY (SELECT * FROM measure) TO STDOUT" -d `, dbMain.DbName)
	...
	bg := &bytes.Buffer{}
	dumpCmd = exec.Command("psql", `-c "COPY (SELECT * FROM measure) TO STDOUT" -d `, dbMain.LiveTestDBName)
	dumpCmd.Env = append(os.Environ(), dbMain.pgEnv()...)
	dumpCmd.Stdout = bg
	err = dumpCmd.Start()
	if err != nil {
	...
	if len(result) > 0 {
		t.Fatalf("MeasuresLive failed but it's probably trivial: %s", strings.Replace(result, "\t", " ", -1))
	}

}

var (
	measureDBTypes = map[string]string{`AuditAction`: `enum.action('attempt','open','close','add','change','remove','superuser')`, `AuditIP`: `inet`, `AuditTime`: `timestamp with time zone`, `AuditUser`: `integer`, `Datum`: `text`, `Metric`: `integer`, `Record`: `integer`}
	_              = bytes.MinRead
)
