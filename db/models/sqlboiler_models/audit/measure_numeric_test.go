...
var (
	measureNumericDBTypes = map[string]string{`AuditAction`: `enum.action('attempt','open','close','add','change','remove','superuser')`, `AuditIP`: `inet`, `AuditTime`: `timestamp with time zone`, `AuditUser`: `integer`, `Datum`: `text`, `Metric`: `integer`, `Record`: `integer`}
	_                     = bytes.MinRead
)
