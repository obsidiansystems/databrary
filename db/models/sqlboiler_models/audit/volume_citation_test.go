...
var (
	volumeCitationDBTypes = map[string]string{`AuditAction`: `enum.action('attempt','open','close','add','change','remove','superuser')`, `AuditIP`: `inet`, `AuditTime`: `timestamp with time zone`, `AuditUser`: `integer`, `Head`: `text`, `URL`: `text`, `Volume`: `integer`, `Year`: `smallint`}
	_                     = bytes.MinRead
)
