...
var (
	volumeAccessDBTypes = map[string]string{`AuditAction`: `enum.action('attempt','open','close','add','change','remove','superuser')`, `AuditIP`: `inet`, `AuditTime`: `timestamp with time zone`, `AuditUser`: `integer`, `Children`: `enum.permission('NONE','PUBLIC','SHARED','READ','EDIT','ADMIN')`, `Individual`: `enum.permission('NONE','PUBLIC','SHARED','READ','EDIT','ADMIN')`, `Party`: `integer`, `Sort`: `smallint`, `Volume`: `integer`}
	_                   = bytes.MinRead
)
