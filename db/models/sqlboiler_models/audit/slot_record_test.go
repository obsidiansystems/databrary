...
var (
	slotRecordDBTypes = map[string]string{`AuditAction`: `enum.action('attempt','open','close','add','change','remove','superuser')`, `AuditIP`: `inet`, `AuditTime`: `timestamp with time zone`, `AuditUser`: `integer`, `Container`: `integer`, `Record`: `integer`, `Segment`: `USER-DEFINED`}
	_                 = bytes.MinRead
)
