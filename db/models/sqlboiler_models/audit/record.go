...
func (q recordQuery) One() (*Record, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "audit: failed to execute a one query for record")
	}
...
func (q recordQuery) All() (RecordSlice, error) {
	...
	err := q.Bind(&o)
	if err != nil {
		return nil, errors.Wrap(err, "audit: failed to assign all query results to Record slice")
	}
...
func (q recordQuery) Count() (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "audit: failed to count record rows")
	}
...
func (q recordQuery) Exists() (bool, error) {
	...
	err := q.Query.QueryRow().Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "audit: failed to check if record exists")
	}
...
func (o *Record) Insert(exec boil.Executor, whitelist ...string) error {
	if o == nil {
		return errors.New("audit: no record provided for insertion")
	}
	...
	if err != nil {
		return errors.Wrap(err, "audit: unable to insert into record")
	}
