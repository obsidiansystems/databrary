...
var (
	slotAssetDBTypes = map[string]string{`Asset`: `integer`, `AuditAction`: `enum.action('attempt','open','close','add','change','remove','superuser')`, `AuditIP`: `inet`, `AuditTime`: `timestamp with time zone`, `AuditUser`: `integer`, `Container`: `integer`, `Segment`: `USER-DEFINED`}
	_                = bytes.MinRead
)
