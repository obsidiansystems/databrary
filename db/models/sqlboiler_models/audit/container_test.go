...
var (
	containerDBTypes = map[string]string{`AuditAction`: `enum.action('attempt','open','close','add','change','remove','superuser')`, `AuditIP`: `inet`, `AuditTime`: `timestamp with time zone`, `AuditUser`: `integer`, `Date`: `date`, `ID`: `integer`, `Name`: `text`, `Top`: `boolean`, `Volume`: `integer`}
	_                = bytes.MinRead
)
