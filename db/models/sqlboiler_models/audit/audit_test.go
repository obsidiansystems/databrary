...
func testAuditsLive(t *testing.T) {
	all, err := Audits(dbMain.liveDbConn).All()
	if err != nil {
		t.Fatalf("failed to get all Audits err: ", err)
	}
	...
	bf := &bytes.Buffer{}
	dumpCmd := exec.Command("psql", `-c "COPY (SELECT * FROM audit) TO STDOUT" -d `, dbMain.DbName)
	...
	bg := &bytes.Buffer{}
	dumpCmd = exec.Command("psql", `-c "COPY (SELECT * FROM audit) TO STDOUT" -d `, dbMain.LiveTestDBName)
	dumpCmd.Env = append(os.Environ(), dbMain.pgEnv()...)
	dumpCmd.Stdout = bg
	err = dumpCmd.Start()
	if err != nil {
	...
	if len(result) > 0 {
		t.Fatalf("AuditsLive failed but it's probably trivial: %s", strings.Replace(result, "\t", " ", -1))
	}

}

var (
	auditDBTypes = map[string]string{`AuditAction`: `enum.action('attempt','open','close','add','change','remove','superuser')`, `AuditIP`: `inet`, `AuditTime`: `timestamp with time zone`, `AuditUser`: `integer`}
	_            = bytes.MinRead
)
