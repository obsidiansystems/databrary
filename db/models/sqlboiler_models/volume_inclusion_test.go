...
func testVolumeInclusionsLive(t *testing.T) {
	...
	bf := &bytes.Buffer{}
	dumpCmd := exec.Command("pg_dump", "--data-only", dbMain.DbName, "-t", "volume_inclusion")
	...
	bg := &bytes.Buffer{}
	dumpCmd = exec.Command("pg_dump", "--data-only", dbMain.LiveTestDBName, "-t", "volume_inclusion")
	dumpCmd.Env = append(os.Environ(), dbMain.pgEnv()...)
	dumpCmd.Stdout = bg
	err = dumpCmd.Start()
	if err != nil {
